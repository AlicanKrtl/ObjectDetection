...
def RandomCrop(img,targ,crop_size=(1000,1000)):
    img_h, img_w = img.shape[1:]

    crop_h = crop_size[0]
    crop_w = crop_size[1]

    crop_search_area = (img_h-crop_h,img_w-crop_h)
    crop_min = (torch.randint(0,crop_search_area[0],size=(1,)),torch.randint(0,crop_search_area[1],size=(1,)))
    crop_max = (crop_min[0]+crop_h,crop_min[1]+crop_w)

    ymin,xmin = crop_min
    ymax,xmax = crop_max

    bbox = targ["boxes"]
    image_id = targ["image_id"]
    labels = targ["labels"]
    iscrowd = targ["iscrowd"]

    image = torchvision.transforms.functional.crop(img,crop_min[0],crop_min[1],crop_size[0],crop_size[1])
    ...
    width = Xmax-Xmin
    height = Ymax - Ymin
    prob = torch.where((width*height)>6000,True,False)
    area = (width*height)[prob]
    label = labels[prob]
    iscrow = iscrowd[prob]
    boxes = torch.stack((Xmin,Ymin,Xmax,Ymax),dim=-1)[prob]
    if torch.max(boxes)>= 1000:
        print(boxes)
        print(torch.max(boxes),torch.min(boxes))
        print(image.shape)
    if len(boxes)>0:
        target = {"boxes":boxes,"labels":label,"image_id":image_id,"area":area,"iscrowd":iscrow}
    else:
        target = {"boxes":torch.tensor([[0,1,2,3]]),"labels":torch.tensor([0]),"image_id":image_id,"area":torch.tensor([4]),"iscrowd":iscrow}

    draw_rect(image,boxes,"train")
    return image,target
